...
1. **Get a session up.** `gateway up --no-prompt`, ask the user for a fresh code,
   ...
   turnaround must be seconds, not a minute.
2. **Run the live acceptance.** `IB_LIVE_TESTS=1 uv run pytest tests/test_live.py -v -s`.
...
3. **Switch `agent-user` to IB Key** (user action, IBKR Mobile). The real fix: it
...
4. **Phase 4 — `export` and `diff`.** Both are in the plan (§6) and absent from
...
5. **Tell the user when a code is wanted.** Nothing notifies today. A systemd user
   timer probing the port and reporting exit 5 would close the gap.
6. **Phase 5 — OH consumes it.** `src/lib/ibagent.ts`, routes fed from ib_agent,
...
7. **In option_harvester, not here:** its write routes have no authentication and
...
- **IBKR throttles repeated logins**: `Too many failed login attempts. Please wait
  58 seconds`. Do not loop `gateway up`.
